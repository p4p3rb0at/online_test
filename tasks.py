...
from codegnan.settings import SERVER_POOL_PORT
...
def start(ctx, ports=SERVER_POOL_PORT, image=SRC_IMAGE_NAME, unsafe=False,
    ...
    if unsafe:
        with ctx.cd(SCRIPT_DIR):
            ...
            base_cmd = "{} -m codegnan.code_server".format(PYTHON_COMMAND)
            run_as_cmd = run_as(OS_NAME)
            cmd = get_cmd(run_as_cmd, base_cmd)
            ctx.run(cmd)
    else:
        cmd_params = {
            ...
            'vol_mount': os.path.join(SCRIPT_DIR, 'codegnan_data'),
            'command': 'sh {0}'.format(
                os.path.join(
                    SCRIPT_DIR,
                    'codegnan_data', 'codegnan', 'scripts', 'codegnan_script.sh')
                )
            }
        ...
        create_dir(os.path.join(SCRIPT_DIR, 'codegnan_data', 'data'))
        create_dir(os.path.join(SCRIPT_DIR, 'codegnan_data', 'output'))

        copy_tree(
            os.path.join(SCRIPT_DIR, 'codegnan'),
            os.path.join(SCRIPT_DIR, 'codegnan_data', 'codegnan')
        )

        copy_file(
            os.path.join(SCRIPT_DIR, 'requirements',
                         'requirements-codeserver.txt'),
            os.path.join(SCRIPT_DIR, 'codegnan_data')
        )
...
def deploy(ctx, fixtures=False, static=True):
    ...
    print("** Setting up & migrating database **")
    base_make_migrate_cmd = "docker exec -i codegnan_django " \
        " {} manage.py makemigrations".format(PYTHON_COMMAND)
    cmd = get_cmd(run_as_cmd, base_make_migrate_cmd)
    ctx.run(cmd)

    base_migrate_cmd = "docker exec -i codegnan_django " \
    ...
    if fixtures:
        base_fixture_cmd = "docker exec -i codegnan_django " \
    ...
    if static:
        base_static_cmd = "docker exec -i codegnan_django " \
