...
# Local Imports
from codegnan.code_server import (
...
FIXTURES_DIR_PATH = os.path.join(settings.BASE_DIR, 'codegnan', 'fixtures')
...
def get_model_class(model):
    ctype = ContentType.objects.get(app_label="codegnan", model=model)
...
def write_static_files_to_zip(zipfile, course_name, current_dir, static_files):
    ...
    for folder in static_files.keys():
        folder_path = os.sep.join((current_dir, "static", "codegnan", folder))
...
class Lesson(models.Model):
    ...
    def _add_lesson_to_zip(self, next_unit, module, course, zip_file, path):
        ...
        unit_file_path = os.sep.join((
            path, "templates", "codegnan", "download_course_templates",
            "unit.html"
            ))
    ...
    def create_demo(self, user):
        ...
        if not course:
            ...
            file_path = os.sep.join(
                (os.path.dirname(__file__), "templates", "codegnan",
                 "demo_video.html")
                )
...
class Question(models.Model):
    ...
    def get_test_cases(self, **kwargs):
        ...
        for tc in self.testcase_set.values_list("type", flat=True).distinct():
            test_case_ctype = ContentType.objects.get(app_label="codegnan",
                                                      model=tc)
